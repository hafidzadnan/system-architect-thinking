...
- `sessionStore.js` — a hardcoded mock `sessions` array; no create/update/delete actions beyond selecting the active session. Also holds the **`phase01`/`phase02`/`phase03` slices**; Phase04–05/Summary are still local component state.
  ...
  - `phase04` — `options` (2–5 policy options), `criteria`, `scores` (`{ [criteriaId]: { [optionId]: { value, probability } } }`), plus `mapStatus`/`mapError`/`mappedAt` for the AI run. Unlike `phase02`, criteria are a **real list, not a 1:1 derivation** — each carries a `sourceVariableId` back to its Fase 02 variable, because README #19 lets the AI split one variable into a Benefit *and* a Cost criterion. `selectPhase04Criteria(state)` joins in the source name and drops criteria whose source variable no longer exists; `selectPhase04Results(state)` runs the EV maths. `cancelPhase04Mapping` mirrors `cancelPhase03Analysis` for the same reason.
  - `phase05` — `zombieStatus`/`zombieJustification`, `metrics` (row array), `interval`, `rollbackTrigger`, `stakeholderQuadrant`/`stakeholderJustification`.
- `settingsStore.js` — OpenRouter API key (defaults from `VITE_OPENROUTER_API_KEY`, see Commands) + selected model, in-memory only.

**Guided session flow** (`SessionPage.jsx` + `components/phases/Phase01–05.jsx` + `SessionSummary.jsx`): the 5-phase wizard (System Requirements → Variable Definition → Logic Debugging → Computation → Output Validation) is driven by **local `useState`** in `SessionPage.jsx` (`activePhase`, `maxUnlockedPhase`), not by `sessionStore` — phase progress (which phase you're on) is not written back to the store and is lost on refresh. Each `PhaseNN.jsx` is a self-contained form. **All five phases and the summary are store-backed** — data survives navigating away and back, but is still lost on a full refresh (no persist middleware).
...
- **Phase04** — "🤖 Mapping Kriteria dengan AI" calls `src/services/criteriaMapper.js` to turn Fase 02 variables into Benefit/Cost criteria (same runner pattern as Phase03). EV is computed live by `src/utils/evCalculator.js` using the **weighted** formula `Σ(Benefit × Bobot × P_success) − Σ(Cost × Bobot × P_risk)` — the PRD's unweighted variant is wrong; only the weighted one reproduces the KPPN case study (7.7 / 57.6). Weights total **1.0 per type separately** (README #9), so a single combined check would false-fail. An invalid total shows `.weight-invalid` and disables "Selesaikan Fase 04" but deliberately still renders the table. When every EV is ≤ 0 no winner is picked; a warning suggests revisiting the Root Objective.
- **Phase05** — store-backed Zombie check (which reads the live Fase 04 best EV rather than hardcoding it), Cobra metric list, and a Cipolla matrix rendered from `STAKEHOLDER_QUADRANTS.map()`.
- **SessionSummary** — reads all five slices and renders `(belum diisi)` for anything a phase left blank. The PDF button is `window.print()` + an `@media print` block in `index.css`, **not** a PDF library (README #3).

**`src/services/`**: the app's external-API/file-parsing layer (no `src/lib` exists). `openrouter.js` owns the shared transport — `requestJson({ apiKey, model, systemPrompt, userContent, signal })` handles the missing-key check, input truncation, JSON-mode fallback (~16% of OpenRouter's catalog rejects `response_format`, so a 400 retries once without it), HTTP-status → `AnalysisError` kind mapping, and fence-stripping JSON extraction. **Add a new AI feature by calling `requestJson` with your own prompt + validator, not by re-implementing fetch** — `analyzeDocument` (Fase 01), `biasAnalyzer.js`'s `analyzeBias` (Fase 03), and `criteriaMapper.js`'s `mapCriteria` (Fase 04) are all thin wrappers over it. `biasAnalyzer.js` builds its user message from live Fase 01+02 data (`buildBiasUserContent`) and normalizes the response **back onto the input variable list** in `validateBiasAnalysis`, so the matrix always matches Fase 02 exactly and a variable the model skipped defaults to `validated` rather than failing the whole run. `criteriaMapper.js` follows the same philosophy but allows 1→N: it matches each returned criterion back by `sourceVariableName`, drops unmatched ones, invents a placeholder criterion for any variable the model ignored, and re-normalizes weights per type so the AI's output is immediately valid. `fileExtractor.js` dispatches by file extension (not MIME type — inconsistent across browsers/OS). `pdfjs-dist` and `xlsx` (SheetJS) are both dynamically `import()`-ed inside `pdfExtractor.js`/`xlsxExtractor.js` so they never touch the initial bundle. **`xlsx` is installed from SheetJS's own CDN tarball** (`package.json` dependency is a URL, not a registry version) because the npm-registry `xlsx` package has been unmaintained since 2022 — Dependabot cannot see or update URL dependencies, so bumping it is manual.

**Gap between docs and code**: README's "Arsitektur File" section describes a larger target structure that only partly exists. `src/utils/` now holds `constants.js` (bias types, tag/threshold options, U/I thresholds, criteria types, option limits, Cipolla quadrants), `eisenhowerLogic.js` (`getQuadrant`, `QUADRANTS`), and `evCalculator.js`; `components/charts/EVComparisonChart.jsx` exists. README's `biasAnalyzer.js`/`criteriaMapper.js` live in `src/services/` instead, next to the other API code. Still missing: `pages/EisenhowerPage.jsx`, `components/session/`, `components/ui/`, `components/eisenhower/`, and `utils/templateData.js`. Treat that section of the README as a target architecture, not current fact — verify a file exists before assuming it does.
